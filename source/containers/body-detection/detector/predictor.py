        ...
        if cls.detector is None:
            if object_detection_model_name == 'ssd_512_resnet50_v1_coco':
                cls.detector = gluon.nn.SymbolBlock.imports(
                    symbol_file=os.path.join(model_root_dir, 'body_detector_ssd_512_resnet50_v1_coco-symbol.json'),
                    input_names=['data'],
                    param_file=os.path.join(model_root_dir, 'body_detector_ssd_512_resnet50_v1_coco-0000.params'),
                    ctx=cls.ctx
                )
            elif object_detection_model_name == 'yolo3_darknet53_coco':
                cls.detector = gluon.nn.SymbolBlock.imports(
                    symbol_file=os.path.join(model_root_dir, 'body_detector_yolo3_darknet53_coco-symbol.json'),
                    input_names=['data'],
                    param_file=os.path.join(model_root_dir, 'body_detector_yolo3_darknet53_coco-0000.params'),
                    ctx=cls.ctx
                )
            elif object_detection_model_name == 'yolo3_mobilenet1.0_coco':
                cls.detector = gluon.nn.SymbolBlock.imports(
                    symbol_file=os.path.join(model_root_dir, 'body_detector_yolo3_mobilenet1.0_coco-symbol.json'),
                    input_names=['data'],
                    param_file=os.path.join(model_root_dir, 'body_detector_yolo3_mobilenet1.0_coco-0000.params'),
                    ctx=cls.ctx
                )
            elif object_detection_model_name == 'faster_rcnn_fpn_resnet101_v1d_coco':
                cls.detector = gluon.nn.SymbolBlock.imports(
                    symbol_file=os.path.join(model_root_dir, 'body_detector_faster_rcnn_fpn_resnet101_v1d_coco-symbol.json'),
                    input_names=['data'],
                    param_file=os.path.join(model_root_dir, 'body_detector_faster_rcnn_fpn_resnet101_v1d_coco-0000.params'),
                    ctx=cls.ctx
                )
            else:
                return None
