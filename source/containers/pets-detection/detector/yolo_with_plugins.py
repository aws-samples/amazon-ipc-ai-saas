...
try:
    ctypes.cdll.LoadLibrary('/opt/program/plugins/libyolo_layer.so')
except OSError as e:
...
def _preprocess_yolo(img, input_shape, letter_box=False):
    """Preprocess an image before TRT YOLO inference.
    # Args
...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        with open(self._model_full_path, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self, model_full_path, category_num=80, letter_box=False, cuda_ctx=None):
